...
import { updateSpecificationSchema, SpecificationUpdate, SpecificationFullResponse } from '@/lib/validations/product/specification-validation';
import { CategoryFullResponse } from '@/lib/validations/product/category-validation';
import { updateSpecification } from "@/lib/actions/catalog/specification.action";
...
const EditSpecificationForm = ({ data, categories }: EditSpecificationFormProps) => {
    // Получаем ID категорий из связей
    const initialCategoryIds = data.categorySpecs?.map(spec => spec.category.id) || [];
    const [selectedCategories, setSelectedCategories] = useState<string[]>(initialCategoryIds);

    const form = useForm<SpecificationUpdate & { categoryIds: string[] }>({
    ...
    // DTO: Преобразуем SpecificationImage в CreatedImageResponse для ImageUploader
    ...
    const handleSubmit = async (values: SpecificationUpdate & { categoryIds: string[] }) => {
    ...
    return (
        <div className="mx-auto py-6">
            <div className="mb-6">
                ...
                <p className="text-muted-foreground">Update the specification details</p>
            </div>
                ...
                >
                    ...
                    <div className='flex gap-4'>
                        ...
                        <Button
                            ...
                            className='flex-1'
                        >
                            {form.formState.isSubmitting ? 'Updating Specification...' : 'Update Specification'}
                        </Button>
                    </div>
                </form>
            </Form>
